...
def write_2_LR_SR_map_file(nav_file, num_lines, LR_name, 
                           LR_coverage_list, LR_uniq_coverage_list, SR_cvrg_threshold,LR_cps_dict, prev_LR_name, LR_idx_dict, LR_SR_mapping_file):
...
import PyPluMA
import PyIO
class MapLongShortPlugin:
 def input(self, inputfile):
   self.parameters = PyIO.readParameters(inputfile)
 def run(self):
   pass
 def output(self, outputfile):
   temp_foldername = PyPluMA.prefix()+"/"+self.parameters["temp_foldername"]
   nav_filename = PyPluMA.prefix()+"/"+self.parameters["nav_filename"]
   LR_filename = PyPluMA.prefix()+"/"+self.parameters["LR_filename"]
   SR_cvrg_threshold = int(self.parameters["SR_cvrg_threshold"])
   Nthread = int(self.parameters["Nthread"])
   sort_max_mem = self.parameters["sort_max_mem"]

################################################################################
# Splitting the nav file
   import subprocess
   num_lines = int(subprocess.getstatusoutput('wc -l ' + nav_filename)[1].split()[0])

   if (num_lines == 0):
    log_print("Error: No short reads was aligned to long read. LSC could not correct any long read sequence.")
    exit(1)
    
   Nsplitline = int(1 + int(num_lines/Nthread))
   ...
   for i in range(Nthread):
    ext_ls.append( '.' + lowercase[int(i/ 26)] + lowercase[int(i) % 26] )
   ...
   LR_SR_mapping_file = open(outputfile+".map",'w')
   if (printSCD):
    LR_SR_coverage_file = open(outputfile+ ".scd",'w')
    LR_uSR_coverage_file = open(outputfile + ".uscd",'w')
    LR_SR_coverage_selected_file = open(outputfile + ".scd.selected",'w')
    LR_uSR_coverage_selected_file = open(outputfile + ".uscd.selected",'w')
   ...
   for line in nav_cvrg_file:
    ...
    if line[0]!='#':
        ...
        if (prev_LR_name != LR_name):
            write_2_LR_SR_map_file(nav_file, num_lines, prev_LR_name, LR_coverage_list, LR_uniq_coverage_list, SR_cvrg_threshold, LR_cps_dict, prev_LR_name, LR_idx_dict, LR_SR_mapping_file)
